...
func runSimulations(
	...
	theoreticalSum := make(map[int]float64)
	empiricalSum := make(map[int]float64)

	for _, c := range cases {
		fmt.Printf("\n%s (Theoretical vs Empirical): n1 from 0 to %d\n", c.name, maxN)
		fmt.Printf("%-4s %-15s %-15s %-10s\n", "n1", "Theoretical P", "Empirical P", "Difference")
		fmt.Println(strings.Repeat("-", 50))

		for n := 0; n <= maxN; n++ {
			theoretical := c.fn(lambda0, lambda1, mu, n)
			key := keyProbability{countName: c.name, nCount: n}
			empirical := float64(wifi_visit_case[key]) / float64(counter)
			diff := theoretical - empirical

			theoreticalSum[n] += theoretical
			empiricalSum[n] += empirical

			fmt.Printf("%-4d %-15.8f %-15.8f %+-.8f\n", n, theoretical, empirical, diff)
		}
	}

	fmt.Println("\n" + strings.Repeat("=", 60))
	fmt.Println("SUM OF ALL CASES FOR EACH n1")
	fmt.Printf("%-4s %-15s %-15s %-10s\n", "n1", "Th. Total P", "Emp. Total P", "Diff")
	fmt.Println(strings.Repeat("-", 50))
	for n := 0; n <= maxN; n++ {
		thTotal := theoreticalSum[n]
		empTotal := empiricalSum[n]
		diff := thTotal - empTotal
		fmt.Printf("%-4d %-15.8f %-15.8f %+-.8f\n", n, thTotal, empTotal, diff)
	}
